...
class MainActivity : FlutterActivity() {
            ...
            when (call.method) {
                "schedule" -> {
                    val id = call.argument<Int>("id")
                        ?: return@setMethodCallHandler result.error("ARG", "Missing id", null)
                    val timeMillis = call.argument<Long>("timeMillis")
                        ?: return@setMethodCallHandler result.error("ARG", "Missing timeMillis", null)
                    val soundName = call.argument<String>("soundName")
                        ?: return@setMethodCallHandler result.error("ARG", "Missing soundName", null)
                    schedulePrePrayerAlarm(
                        id,
                        timeMillis,
                        soundName,
                        call.argument<String>("title") ?: "استعد للصلاة",
                        call.argument<String>("body") ?: "حان وقت الاستعداد للصلاة."
                    )
                    result.success(true)
                }
                "scheduleAdhan" -> {
    ...
    private fun schedulePrePrayerAlarm(
        id: Int,
        timeMillis: Long,
        soundName: String,
        title: String,
        ...
        schedulePrePrayerAlarm(
            ...
            title,
            body,
            PrePrayerAlarmReceiver.DEFAULT_NOTIFICATION_ID
        )
    }
    ...
    private fun cancelPrePrayerAlarm(id: Int) {
        ...
        val pi = PendingIntent.getBroadcast(
            this,
            id,
            Intent(this, PrePrayerAlarmReceiver::class.java),
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
        )
        am.cancel(pi)
        pi.cancel()
    ...
    private fun cancelAdhanAlarm(id: Int) {
        ...
        val pi = PendingIntent.getBroadcast(
            this,
            id,
            Intent(this, AdhanAlarmReceiver::class.java),
            PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE
        )
        am.cancel(pi)
        pi.cancel()
    ...
    private fun cancelAllAdhanAlarms() {
        for (prayerIndex in 0..4) {
            cancelAdhanAlarm(prayerIndex * 10 + 2)
        }
        // Disabling Adhan must also silence an Adhan that is already playing.
        stopService(Intent(this, AdhanAlarmService::class.java))
    }
    ...
    private fun isIgnoringBatteryOptimizations(): Boolean {
        ...
        return pm.isIgnoringIgnoringBatteryOptimizationsCompat(packageName)
    }
...
private fun PowerManager.isIgnoringIgnoringBatteryOptimizationsCompat(packageName: String): Boolean =
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.M) true else isIgnoringBatteryOptimizations(packageName)
