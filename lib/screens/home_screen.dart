...
class _HomeScreenState extends State<HomeScreen> {
  ...
  String _identityMessage(AppState state) {
    if (state.streak >= 21) {
      return 'أنت من المحافظين على الصلاة';
    }

    if (state.streak >= 7) {
      return 'أنت شخص يحافظ على ${state.activePrayers.first.arabicName}';
    }

    if (state.streak >= 1) {
      return 'بداية موفقة — استمر بنفس الوتيرة';
    }
  ...
  String _locationLabel(AppState state) {
    if (!state.notificationsActive) {
      return 'الإشعارات غير مفعّلة';
    }

    if (state.timesLoading) {
  ...
  ///
  /// Returns null when real prayer times are not available yet.
  ...
  Widget build(BuildContext context) {
    ...
    // IMPORTANT:
    // This is the Aqim-specific enum from prayer_window_icon.dart.
        ...
        child: Padding(
          ...
          child: Column(
            children: [
              ...
              _TitleBlock(
                state: state,
                locationLabel: _locationLabel(state),
              ),
              ...
              child: Row(
                ...
                children: [
                  Text(
                    ...
                    style: TextStyle(
                      fontSize: 10.5,
                      color: state.notificationsActive
                          ? Colors.white70
                          : AppColors.goldSoft,
