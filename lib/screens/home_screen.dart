...
class _HomeScreenState extends State<HomeScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance
        .addPostFrameCallback((_) => _maybePromptBatterySettings());
  }
  ...
  Future<void> _maybePromptBatterySettings() async {
    ...
    final openSettings = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        title: Text('لضمان وصول تذكيراتك فوقتها',
        ...
        content: Text(
            ...
            style: GoogleFonts.cairo(
                color: AppColors.inkSoft, height: 1.7, fontSize: 13.5)),
        actions: [
          TextButton(
  ...
  String _locationLabel(AppState state) {
    if (state.timesLoading && state.cityName == null)
...
class _TopRow extends StatelessWidget {
  ...
  Widget build(BuildContext context) => SizedBox(
        height: 46,
        child: Row(children: [
          Image.asset('assets/images/aqim_logo_transparent_512.png',
          ...
          Container(
            ...
            child: Row(mainAxisSize: MainAxisSize.min, children: [
              const Icon(Icons.local_fire_department_rounded,
                  size: 15, color: AppColors.goldSoft),
              const SizedBox(width: 5),
              Text('سلسلة ${state.streak}',
      ...
      child: Row(children: [
        Expanded(
            child: Column(
                ...
                children: [
              Text(GregorianArabic.format(DateTime.now()),
                  style: GoogleFonts.cairo(
                      fontSize: 11.5,
                      fontWeight: FontWeight.w700,
                      color: AppColors.ivory)),
              const SizedBox(height: 5),
              Text(HijriDate.fromGregorian(DateTime.now()).formatted,
                  style: GoogleFonts.cairo(
        ...
        Column(
            ...
            children: [
              Image.asset('assets/images/aqim_logo_transparent_512.png',
                  width: 92, height: 66, fit: BoxFit.contain),
              Text('لأجل صلاة في وقتها',
                  style: GoogleFonts.cairo(
                      fontSize: 10.5,
      ...
      child: InkWell(
        ...
        onTap: () {
          if (missed.length == 1) {
            Navigator.of(context).push(MaterialPageRoute(
                builder: (_) =>
                    MissedPrayerResponseScreen(prayer: missed.first)));
            return;
          }
                    ...
                    child: Padding(
                        ...
                        child:
                          ...
                          ...missed.map((p) => ListTile(
                              ...
                              title: Text('فاتتك صلاة ${p.arabicName}',
                                  style: GoogleFonts.cairo(
                                      color: AppColors.ember,
                                      fontWeight: FontWeight.w800)),
                              ...
                              onTap: () {
                                ...
                                Navigator.of(context).push(MaterialPageRoute(
                                    builder: (_) =>
                                        MissedPrayerResponseScreen(prayer: p)));
                              }))
                        ]))));
          }
        },
        child: Container(
          ...
          decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(20),
              border: Border.all(
                  color: AppColors.ember.withOpacity(.88), width: 1.3),
              boxShadow: [
                BoxShadow(
                    color: AppColors.ember.withOpacity(.10),
                    blurRadius: 18,
                    spreadRadius: 1)
              ]),
          child: Row(crossAxisAlignment: CrossAxisAlignment.center, children: [
            Container(
                ...
                decoration: BoxDecoration(
                    ...
                    color: AppColors.ember.withOpacity(.10),
                    border: Border.all(
                        color: AppColors.ember.withOpacity(.85), width: 1.2)),
                child: const Icon(Icons.notifications_active_rounded,
                    color: AppColors.ember, size: 31)),
            const SizedBox(width: 14),
            Expanded(
                child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                  Text('تنبيه الصلاة الفائتة',
                      style: GoogleFonts.amiri(
                          fontSize: 18,
                          fontWeight: FontWeight.w800,
                          color: AppColors.ember)),
                  const SizedBox(height: 3),
                  ...missed.map((prayer) => Text(
                      ...
                      style: GoogleFonts.cairo(
                          ...
                          height: 1.35))),
                  const SizedBox(height: 5),
                  Text('اضغط هنا للإجابة مباشرة: هل صليتها أم لا؟',
                      style: GoogleFonts.cairo(
                          fontSize: 10.5,
                          color: AppColors.inkSoft,
                          height: 1.35))
...
class _HeroCard extends StatelessWidget {
  ...
  const _HeroCard(
      {required this.state, required this.next, required this.period});
  @override
  ...
  Widget build(BuildContext context) => Row(children: [
        ...
        Expanded(
            child: _QuickActionCard(
                ...
                button: 'عرض التقرير',
                onTap: () => AppInterstitialAd.showThen(() =>
                    Navigator.of(context).push(MaterialPageRoute(
...
class _QiblaQuickAction extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Material(
      ...
      child: InkWell(
          ...
          onTap: () => AppInterstitialAd.showThen(() => Navigator.of(context)
                    ...
                    child: Column(
                      ...
                      Text('تحديد القبلة',
                          style: GoogleFonts.amiri(
                              ...
                              fontWeight: FontWeight.w800,
                              color: AppColors.ivory)),
                      Text('بوصلة حية باتجاه الكعبة من موقعك',
                          style: GoogleFonts.cairo(
                              fontSize: 10.5, color: AppColors.inkSoft))
                    ])),
                const Icon(Icons.chevron_left_rounded,
...
class _WeeklyProgressCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    int progressFor(DateTime day) {
      final key = _key(day);
      if (key == _key(now)) {
        final done = state.activePrayers
            .where((p) => state.todayStatus[p] == PrayerStatus.done)
            .length;
        return state.activePrayers.isEmpty
            ? 0
            : ((done / state.activePrayers.length) * 100).round();
      }
                    ...
                    children: [
                      Text(_weekday(day.weekday),
                          ...
                          style: GoogleFonts.cairo(
                              ...
                              color:
                                  isToday ? AppColors.gold : AppColors.ivory)),
                      const SizedBox(height: 2),
                      ...
                      Container(
                        ...
                        decoration: BoxDecoration(
                          ...
                          border: Border.all(
                              color: isToday
                                  ? AppColors.gold.withOpacity(.9)
                                  : AppColors.paperLine.withOpacity(.55),
                              width: isToday ? 1.5 : 1),
                        ),
