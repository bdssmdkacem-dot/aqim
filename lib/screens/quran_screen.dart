  ...
  Future<void> _saveProgress() async {
    ...
    try {
      ...
      if (_page >= 604) {
        ...
        await prefs.setString('quran_last_khatmah_at', DateTime.now().toIso8601String());
        ...
        if (!mounted) return;
        setState(() { _completedKhatmahCount = count; _savedPage = 1; _saving = false; });
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('ما شاء الله! أتممت الختمة رقم $count. تقبّل الله منك 🌙'),
      ...
      if (!mounted) return;
      setState(() { _savedPage = _page; _saving = false; });
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تعذّر حفظ التقدم. حاول مرة أخرى.')));
    }
  }
            ...
            child: Column(children: [
              Padding(
                ...
                child: Row(children: [
                  ...
                  Text('فهرس القرآن الكريم', style: GoogleFonts.amiri(color: AppColors.ivory, fontSize: 23, fontWeight: FontWeight.w800)),
                ]),
              ),
              Padding(
                padding: const EdgeInsets.symmetric(horizontal: 20),
                child: Text('اختر السورة للانتقال مباشرة إلى أول صفحة منها', style: GoogleFonts.cairo(color: AppColors.textMuted, fontSize: 12)),
              ),
              ...
              Expanded(
                child: ListView.separated(
                  ...
                  separatorBuilder: (_, __) => Divider(color: AppColors.paperLine.withOpacity(.45), height: 1),
                  itemBuilder: (_, i) {
                    final s = _surahs![i];
                    return ListTile(
                      ...
                      leading: Container(
                        width: 48, height: 48,
                        decoration: BoxDecoration(shape: BoxShape.circle, color: AppColors.gold.withOpacity(.10), border: Border.all(color: AppColors.gold.withOpacity(.30))),
                        alignment: Alignment.center,
                        child: Text('${s.number}', style: GoogleFonts.cairo(color: AppColors.gold, fontWeight: FontWeight.w800)),
                      ),
                      title: Text(s.name, textAlign: TextAlign.right, style: GoogleFonts.amiri(fontSize: 21, color: AppColors.ivory, fontWeight: FontWeight.w700)),
                      subtitle: Text('${s.englishName}  •  ${s.numberOfAyahs} آية', textAlign: TextAlign.right, style: const TextStyle(color: AppColors.textMuted, fontSize: 11)),
                      ...
                      onTap: () async {
                        ...
                        try {
                          ...
                          if (mounted) ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('تعذّر فتح بداية السورة.')));
                        }
                      },
                    );
                  },
      ...
      builder: (sheetContext) => StatefulBuilder(
        builder: (context, setSheetState) {
          Future<void> runSearch() async {
            ...
            setSheetState(() { searching = true; searchError = null; results = const []; });
            try {
              final found = await QuranService.instance.search(query);
              setSheetState(() { results = found; searching = false; });
            } catch (_) {
              setSheetState(() { searching = false; searchError = 'تعذّر البحث الآن. تحقق من الاتصال وحاول مرة أخرى.'; });
            }
          }
          return SafeArea(
            child: Padding(
              padding: EdgeInsets.fromLTRB(16, 14, 16, 12 + MediaQuery.of(context).viewInsets.bottom),
              child: SizedBox(
                ...
                child: Column(children: [
                  Row(children: [
                    ...
                    Text('البحث في القرآن الكريم', style: GoogleFonts.amiri(color: AppColors.ivory, fontSize: 22, fontWeight: FontWeight.w800)),
                  ]),
                  ...
                  TextField(
                    ...
                    decoration: InputDecoration(
                      ...
                      prefixIcon: IconButton(onPressed: runSearch, icon: const Icon(Icons.search_rounded), color: AppColors.gold),
                      filled: true,
                      fillColor: AppColors.ink.withOpacity(.55),
                      border: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide(color: AppColors.gold.withOpacity(.18))),
                      enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide(color: AppColors.gold.withOpacity(.18))),
                  ...
                  if (searching)
                    ...
                    Padding(padding: const EdgeInsets.all(20), child: Text(searchError!, textAlign: TextAlign.center, style: GoogleFonts.cairo(color: AppColors.textMuted)))
                  else if (controller.text.trim().isNotEmpty && results.isEmpty)
                    Padding(padding: const EdgeInsets.all(20), child: Text('لا توجد آيات مطابقة.', style: GoogleFonts.cairo(color: AppColors.textMuted)))
                  else
                    Expanded(
                      child: ListView.separated(
                        itemCount: results.length,
                        separatorBuilder: (_, __) => Divider(color: AppColors.paperLine.withOpacity(.35), height: 1),
                        itemBuilder: (_, i) {
                          final verse = results[i].verse;
                          return ListTile(
                            contentPadding: const EdgeInsets.symmetric(horizontal: 4, vertical: 5),
                            title: Text(verse.text, textAlign: TextAlign.right, maxLines: 4, overflow: TextOverflow.ellipsis, style: GoogleFonts.amiri(color: AppColors.ivory, fontSize: 19, height: 1.7)),
                            subtitle: Text('${verse.surahName} • الآية ${verse.numberInSurah} • الصفحة ${verse.page}', textAlign: TextAlign.right, style: GoogleFonts.cairo(color: AppColors.gold, fontSize: 11)),
        ...
        child: Padding(
          ...
          child: FutureBuilder<QuranTafsir>(
            ...
            builder: (context, snapshot) {
              if (snapshot.connectionState == ConnectionState.waiting) return const SizedBox(height: 220, child: Center(child: CircularProgressIndicator(color: AppColors.gold)));
              if (snapshot.hasError || !snapshot.hasData) return SizedBox(height: 220, child: Center(child: Text('تعذّر تحميل التفسير الآن. حاول مرة أخرى.', textAlign: TextAlign.center, style: GoogleFonts.cairo(color: AppColors.textMuted))));
                ...
                child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
                  Row(children: [
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                      ...
                      Text('${verse.surahName} • الآية ${verse.numberInSurah}', textAlign: TextAlign.right, style: GoogleFonts.cairo(color: AppColors.textMuted, fontSize: 11)),
                    ])),
                  ]),
                  ...
                  Text(tafsir.text, textAlign: TextAlign.right, style: GoogleFonts.cairo(color: AppColors.ivory, fontSize: 15, height: 1.9)),
                ]),
              );
            },
          ),
    ...
    await showModalBottomSheet<void>(
      ...
      builder: (_) => SafeArea(
        child: SingleChildScrollView(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            Row(children: [
              Container(width: 46, height: 46, decoration: BoxDecoration(shape: BoxShape.circle, color: AppColors.gold.withOpacity(.12), border: Border.all(color: AppColors.gold.withOpacity(.40))), child: const Icon(Icons.self_improvement_rounded, color: AppColors.gold)),
              const SizedBox(width: 12),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                Text('سجدة التلاوة', textAlign: TextAlign.right, style: GoogleFonts.amiri(color: AppColors.ivory, fontSize: 24, fontWeight: FontWeight.w800)),
                Text('${verse.surahName} • الآية ${verse.numberInSurah}', textAlign: TextAlign.right, style: GoogleFonts.cairo(color: AppColors.textMuted, fontSize: 11)),
              ])),
            ]),
            const SizedBox(height: 18),
            ...
            ...[
              ...
              child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Container(width: 28, height: 28, decoration: BoxDecoration(shape: BoxShape.circle, color: AppColors.gold.withOpacity(.12)), alignment: Alignment.center, child: Text('${entry.key + 1}', style: GoogleFonts.cairo(color: AppColors.gold, fontWeight: FontWeight.w800, fontSize: 11))),
                const SizedBox(width: 10),
                Expanded(child: Text(entry.value, textAlign: TextAlign.right, style: GoogleFonts.cairo(color: AppColors.ivory, fontSize: 14, height: 1.7))),
              ]),
            )),
            const SizedBox(height: 4),
            Text('ملاحظة: قد تختلف بعض التفاصيل الفقهية باختلاف المذهب، فاستعن بما تتبعه من أحكام.', textAlign: TextAlign.right, style: GoogleFonts.cairo(color: AppColors.textMuted, fontSize: 11, height: 1.7)),
          ]),
        ),
      ),
    );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(onPressed: _openSearch, icon: const Icon(Icons.search_rounded), tooltip: 'البحث في القرآن'),
      ...
      body: Column(children: [
        if (page != null && !_loading)
          Padding(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(color: AppColors.surfaceDark, borderRadius: BorderRadius.circular(16), border: Border.all(color: AppColors.gold.withOpacity(.18))),
              child: Row(children: [
                Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.end, children: [
                  Text(page.surahName.isEmpty ? 'القرآن الكريم' : page.surahName, textAlign: TextAlign.right, style: GoogleFonts.amiri(color: AppColors.ivory, fontSize: 20, fontWeight: FontWeight.w800)),
                  Text('الجزء ${page.juz}  •  الحزب ${page.hizb}  •  الصفحة $_page من 604', textAlign: TextAlign.right, style: GoogleFonts.cairo(color: AppColors.textMuted, fontSize: 11)),
                ])),
                const SizedBox(width: 12),
        ...
        if (_savedPage != null && _savedPage == _page && !_loading)
          Padding(padding: const EdgeInsets.fromLTRB(16, 0, 16, 6), child: Row(children: [
            const Icon(Icons.bookmark_rounded, color: AppColors.gold, size: 17),
            const SizedBox(width: 6),
            Expanded(child: Text('تقدمك محفوظ هنا — ستعود إلى الصفحة $_page عند فتح القرآن مرة أخرى.', textAlign: TextAlign.right, style: GoogleFonts.cairo(color: AppColors.textMuted, fontSize: 10))),
          ])),
        if (isLastPage)
          Padding(padding: const EdgeInsets.fromLTRB(16, 0, 16, 8), child: Container(
            width: double.infinity,
            padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10),
            decoration: BoxDecoration(color: AppColors.gold.withOpacity(.10), borderRadius: BorderRadius.circular(15), border: Border.all(color: AppColors.gold.withOpacity(.35))),
            child: Row(children: [
              const Icon(Icons.flag_rounded, color: AppColors.gold),
              const SizedBox(width: 10),
              Expanded(child: Text('نهاية الختمة — احفظ الصفحة لإتمام الختمة وبدء ختمة جديدة.', textAlign: TextAlign.right, style: GoogleFonts.cairo(color: AppColors.ivory, fontSize: 12, fontWeight: FontWeight.w700))),
            ]),
          )),
        Padding(padding: const EdgeInsets.fromLTRB(18, 0, 18, 5), child: Row(children: [
          Text('${(progress * 100).round()}%', style: GoogleFonts.cairo(color: AppColors.gold, fontWeight: FontWeight.w800, fontSize: 11)),
          const Spacer(),
          Text(_completedKhatmahCount > 0 ? 'ختمات مكتملة: $_completedKhatmahCount' : 'وردك اليومي', style: GoogleFonts.cairo(color: AppColors.textMuted, fontSize: 11)),
        ])),
        LinearProgressIndicator(value: progress, minHeight: 4, backgroundColor: AppColors.paperLine, color: AppColors.gold),
