...
class _QuranScreenState extends State<QuranScreen> {
          ...
          child: SizedBox(
            ...
            child: ListView.separated(
              ...
              separatorBuilder: (_, __) => Divider(
                color: AppColors.paperLine.withOpacity(.5),
              ),
              itemBuilder: (_, i) {
                final s = _surahs![i];
                return ListTile(
                  leading: CircleAvatar(
                    backgroundColor: AppColors.gold.withOpacity(.12),
                    child: Text(
                      '${s.number}',
  ...
  Future<void> _markPageRead() async {
    ...
    if (!mounted) return;
    final message = _page >= 604
        ? 'ما شاء الله! أتممت ختمة القرآن، وبدأت ختمة جديدة 🌙'
        : 'أحسنت! تم حفظ تقدمك. غدًا نكمل من الصفحة $next.';
    ScaffoldMessenger.of(context).showSnackBar(
                ...
                children: [
                  ...
                  Expanded(
                    child: ElevatedButton.icon(
                      ...
                      label: const Text('قرأت هذه الصفحة'),
                    ),
                  ),
                  const SizedBox(width: 8),
                  Expanded(
