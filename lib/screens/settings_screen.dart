...
class _SettingsContentScreenState extends State<_SettingsContentScreen>
  ...
  Future<void> _previewAdhan({required bool fajr}) async {
    ...
    if (playing) {
      ...
      if (mounted)
        setState(
            () => fajr ? _playingFajrAdhan = false : _playingAdhan = false);
      return;
    }
    ...
    try {
      await AudioService.instance.playAsset(context,
          fajr ? 'adhan/$_selectedFajrAdhan.mp3' : 'adhan/$_selectedAdhan.mp3');
    } finally {
      if (mounted)
        setState(
            () => fajr ? _playingFajrAdhan = false : _playingAdhan = false);
  ...
  Widget _buildTimingCard(AppState state) => Card(
      child: Padding(
          padding: const EdgeInsets.all(16),
          child:
              Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            const Text('وقت التذكير قبل الصلاة',
                style: TextStyle(fontWeight: FontWeight.w800)),
            const SizedBox(height: 6),
            const Text('متى تريد أن يذكّرك أقم بالاستعداد؟',
                style: TextStyle(fontSize: 12, color: AppColors.textMuted)),
            const SizedBox(height: 12),
            Wrap(
                spacing: 8,
                runSpacing: 8,
                children: _beforeOptions
                    .map((minutes) => ChoiceChip(
                        label: Text('$minutes د'),
                        selected: state.beforeMinutes == minutes,
                        onSelected: (_) =>
                            state.updateReminderTiming(before: minutes)))
                    .toList()),
            const Divider(height: 28),
            const Text('تذكير «هل صليت؟»',
                style: TextStyle(fontWeight: FontWeight.w800)),
            const SizedBox(height: 8),
            Wrap(
                spacing: 8,
                runSpacing: 8,
                children: _afterOptions
                    .map((minutes) => ChoiceChip(
                        label: Text('$minutes د'),
                        selected: state.afterMinutes == minutes,
                        onSelected: (_) =>
                            state.updateReminderTiming(after: minutes)))
                    .toList()),
          ])));

  Widget _buildPrePrayerCard() => Card(
      child: Padding(
          ...
          child: Column(children: [
            SwitchListTile.adaptive(
                ...
                title: const Text('تنبيهات قبل الصلاة',
                    style: TextStyle(fontWeight: FontWeight.w800)),
                subtitle: const Text(
                    'منبه مستقل عن الأذان؛ يستخدم أصوات alarm الخاصة بكل صلاة.',
                    style: TextStyle(fontSize: 12))),
            if (_prePrayerEnabled) ...[
              const Divider(height: 20),
              const Align(
                  alignment: Alignment.centerRight,
                  child: Text('الصلاة التي تريد التنبيه لها',
                      style: TextStyle(fontWeight: FontWeight.w700))),
              const SizedBox(height: 10),
              Wrap(
                  spacing: 7,
                  runSpacing: 7,
                  children: _prayerNames.entries
                      .map((entry) => FilterChip(
                          label: Text(entry.value),
                          selected: _prePrayerPrayers.contains(entry.key.name),
              ...
              DropdownButtonFormField<String>(
                  ...
                  decoration: const InputDecoration(
                      labelText: 'طريقة تنبيه ما قبل الصلاة',
                      prefixIcon: Icon(Icons.notifications_active_rounded)),
                  items: _prePrayerModes.entries
              ...
              Text(
                  _prePrayerMode == 'alarm'
                      ? 'يستخدم صوت alarm المخصص للصلاة القادمة، وليس الأذان.'
  ...
  Widget _buildAdhanCard(AppState state) => Card(
      child: Padding(
          padding: const EdgeInsets.all(16),
          child:
            ...
            SwitchListTile.adaptive(
                ...
                title: const Text('الأذان عند دخول وقت الصلاة',
                    style: TextStyle(fontWeight: FontWeight.w800)),
                subtitle: const Text(
                    'إعداد مستقل تمامًا عن تنبيهات alarm قبل الصلاة.',
                    style: TextStyle(fontSize: 12))),
            const Divider(height: 20),
            DropdownButtonFormField<String>(
            ...
            if (_adhanAlertMode == 'adhan') ...[
              ...
              const Text('أذان الفجر',
                  style: TextStyle(fontWeight: FontWeight.w900)),
              const SizedBox(height: 8),
              DropdownButtonFormField<String>(
                  initialValue: _fajrAdhanSounds.containsKey(_selectedFajrAdhan)
                      ? _selectedFajrAdhan
                      : _fajrAdhanSounds.keys.first,
                  decoration: const InputDecoration(
                      labelText: 'اختر أذان الفجر',
                      prefixIcon: Icon(Icons.wb_twilight_rounded)),
                  items: _fajrAdhanSounds.entries
                      .map((entry) => DropdownMenuItem(
                          value: entry.key, child: Text(entry.value)))
                      .toList(),
                  onChanged: (value) {
                    if (value != null) _selectFajrAdhan(value);
                  }),
              const SizedBox(height: 10),
              OutlinedButton.icon(
                  onPressed:
                      _loadingAudio ? null : () => _previewAdhan(fajr: true),
                  icon: Icon(_playingFajrAdhan
                      ? Icons.stop_circle_rounded
                      : Icons.play_circle_fill_rounded),
                  label: Text(_playingFajrAdhan
                      ? 'إيقاف أذان الفجر'
                      : 'تشغيل أذان الفجر')),
              const Divider(height: 28),
              const Text('أذان الصلوات الأخرى',
                  style: TextStyle(fontWeight: FontWeight.w900)),
              const SizedBox(height: 8),
              DropdownButtonFormField<String>(
                  initialValue: _adhanSounds.containsKey(_selectedAdhan)
                      ? _selectedAdhan
                      : _adhanSounds.keys.first,
                  decoration: const InputDecoration(
                      labelText: 'اختر مؤذن الصلوات الأخرى',
              ...
              OutlinedButton.icon(
                  onPressed:
                      _loadingAudio ? null : () => _previewAdhan(fajr: false),
                  icon: Icon(_playingAdhan
                      ? Icons.stop_circle_rounded
                      : Icons.play_circle_fill_rounded),
                  label: Text(_playingAdhan ? 'إيقاف الأذان' : 'تشغيل الأذان')),
              const SizedBox(height: 6),
              const Text('يمكنك تجربة كل صوت وتشغيله أو إيقافه قبل اعتماده.',
              ...
              Padding(
                  ...
                  child: Text(
                      _adhanAlertMode == 'ringtone'
                          ? 'سيستخدم رنة الهاتف عند دخول وقت الصلاة.'
                          : 'سيستخدم الاهتزاز فقط عند دخول وقت الصلاة.',
                      textAlign: TextAlign.center,
                      style: const TextStyle(
                          fontSize: 12, color: AppColors.textMuted))),
          ])));
          ...
          child:
            ...
            Row(children: [
              Icon(
                  ...
                  color: ready ? AppColors.sage : AppColors.ember),
              const SizedBox(width: 10),
              Expanded(
                  child: Text(
                      ready ? 'إعدادات الخلفية مناسبة' : 'قد تتأخر التنبيهات',
              ...
              if (checking)
                ...
                IconButton(
                    onPressed: onCheck, icon: const Icon(Icons.refresh_rounded))
            ]),
            const SizedBox(height: 8),
            Text(
                ready
                    ? 'يمكن لأقم العمل في الخلفية وإرسال التنبيهات في وقتها.'
                    : 'لضمان وصول تنبيهات الصلاة، اسمح لأقم بالعمل دون قيود من البطارية.',
                style:
            ...
            OutlinedButton.icon(
                ...
                label: const Text('التشغيل التلقائي')),
            const SizedBox(height: 8),
            OutlinedButton.icon(
