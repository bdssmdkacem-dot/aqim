...
class _SettingsScreenState extends State<SettingsScreen> with WidgetsBindingObserver {
  ...
  Future<void> _selectFajrAdhan(String value) async {
    final prefs = await SharedPreferences.getInstance();
    await prefs.setString('adhan_fajr_sound', value);
    if (!mounted) return;
    setState(() => _selectedFajrAdhan = value);
    await context.read<AppState>().loadPrayerTimes();
  }
  ...
  Future<void> _previewAdhan({required bool fajr}) async {
    ...
    if (playing) {
      ...
      if (mounted) setState(() => fajr ? _playingFajrAdhan = false : _playingAdhan = false);
      return;
    }
    if (_loadingAudio) return;
    setState(() {
      _playingAdhan = !fajr;
      _playingFajrAdhan = fajr;
    });
    try {
      final asset = fajr ? 'adhan/$_selectedFajrAdhan.mp3' : 'adhan/$_selectedAdhan.mp3';
      await AudioService.instance.playAsset(context, asset);
    } finally {
      if (mounted) setState(() => fajr ? _playingFajrAdhan = false : _playingAdhan = false);
    }
  }
  ...
  void _togglePrayer(Prayer prayer) {
    setState(() {
      if (_prePrayerPrayers.contains(prayer.name)) { _prePrayerPrayers.remove(prayer.name); } else { _prePrayerPrayers.add(prayer.name); }
    });
        ...
        child: Column(children: [
          Expanded(child: ListView(
            ...
            children: [
              ...
              const _SectionTitle(icon: Icons.notifications_active_rounded, title: 'تنبيهات أقم', subtitle: 'اجعل أقم يذكّرك بالصلاة بالطريقة التي تناسبك.'),
              const SizedBox(height: 10), _buildTimingCard(state), const SizedBox(height: 12), _buildPrePrayerCard(), const SizedBox(height: 12), _buildAdhanCard(state), const SizedBox(height: 22),
              const _SectionTitle(icon: Icons.battery_saver_rounded, title: 'العمل في الخلفية', subtitle: 'يساعد ذلك أقم على إرسال التنبيهات في وقتها.'),
  ...
  Widget _buildPrePrayerCard() => Card(child: Padding(padding: const EdgeInsets.fromLTRB(14, 8, 14, 14), child: Column(children: [
    ...
    if (_prePrayerEnabled) ...[
      ...
      Wrap(spacing: 7, runSpacing: 7, children: _prayerNames.entries.map((entry) => FilterChip(label: Text(entry.value), selected: _prePrayerPrayers.contains(entry.key.name), onSelected: (_) => _togglePrayer(entry.key))).toList()),
      ...
      Text(_prePrayerMode == 'alarm' ? 'يستخدم صوت alarm المخصص للصلاة القادمة، وليس الأذان.' : _prePrayerMode == 'ringtone' ? 'يستخدم رنة الهاتف.' : 'يستخدم الاهتزاز فقط.', style: const TextStyle(fontSize: 12, color: AppColors.textMuted)),
    ],
  ])));
  ...
  Widget _buildAdhanCard(AppState state) => Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
    SwitchListTile.adaptive(contentPadding: EdgeInsets.zero, value: state.adhanEnabled, onChanged: state.setAdhanEnabled, title: const Text('الأذان عند دخول وقت الصلاة', style: TextStyle(fontWeight: FontWeight.w800)), subtitle: const Text('إعداد مستقل تمامًا عن تنبيهات alarm قبل الصلاة.', style: TextStyle(fontSize: 12))),
    ...
    if (_adhanAlertMode == 'adhan') ...[
      ...
      DropdownButtonFormField<String>(initialValue: _fajrAdhanSounds.containsKey(_selectedFajrAdhan) ? _selectedFajrAdhan : _fajrAdhanSounds.keys.first, decoration: const InputDecoration(labelText: 'اختر أذان الفجر', prefixIcon: Icon(Icons.wb_twilight_rounded)), items: _fajrAdhanSounds.entries.map((entry) => DropdownMenuItem(value: entry.key, child: Text(entry.value))).toList(), onChanged: (value) { if (value != null) _selectFajrAdhan(value); }),
      const SizedBox(height: 10),
      OutlinedButton.icon(onPressed: _loadingAudio ? null : () => _previewAdhan(fajr: true), icon: Icon(_playingFajrAdhan ? Icons.stop_circle_rounded : Icons.play_circle_fill_rounded), label: Text(_playingFajrAdhan ? 'إيقاف أذان الفجر' : 'تشغيل أذان الفجر')),
      ...
      const SizedBox(height: 10),
      OutlinedButton.icon(onPressed: _loadingAudio ? null : () => _previewAdhan(fajr: false), icon: Icon(_playingAdhan ? Icons.stop_circle_rounded : Icons.play_circle_fill_rounded), label: Text(_playingAdhan ? 'إيقاف الأذان' : 'تشغيل الأذان')),
      ...
      Padding(padding: const EdgeInsets.only(top: 12), child: Text(_adhanAlertMode == 'ringtone' ? 'سيستخدم رنة الهاتف عند دخول وقت الصلاة.' : 'سيستخدم الاهتزاز فقط عند دخول وقت الصلاة.', textAlign: TextAlign.center, style: const TextStyle(fontSize: 12, color: AppColors.textMuted))),
  ])));
}
...
class _StatusCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Card(color: active ? AppColors.sage.withOpacity(.08) : AppColors.ember.withOpacity(.08), child: Padding(padding: const EdgeInsets.all(16), child: Row(children: [
      Icon(active ? Icons.notifications_active_rounded : Icons.notifications_off_rounded, color: active ? AppColors.sage : AppColors.ember, size: 24), const SizedBox(width: 12),
      Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(active ? 'تنبيهات أقم تعمل' : 'تنبيهات أقم تحتاج إلى تفعيل', style: TextStyle(fontWeight: FontWeight.w900, color: active ? AppColors.sage : AppColors.ember)), const SizedBox(height: 4), Text(state.cityName == null ? 'حدّد موقعك للحصول على أوقات الصلاة الدقيقة.' : 'الموقع: ${state.cityName}', style: const TextStyle(fontSize: 12))])),
...
class _BatteryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Card(child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
    Row(children: [Icon(ready ? Icons.check_circle_rounded : Icons.warning_amber_rounded, color: ready ? AppColors.sage : AppColors.ember), const SizedBox(width: 10), Expanded(child: Text(ready ? 'إعدادات الخلفية مناسبة' : 'قد تتأخر التنبيهات', style: const TextStyle(fontWeight: FontWeight.w800))), if (checking) const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2)) else IconButton(onPressed: onCheck, icon: const Icon(Icons.refresh_rounded))]),
    const SizedBox(height: 8), Text(ready ? 'يمكن لأقم العمل في الخلفية وإرسال التنبيهات في وقتها.' : 'لضمان وصول تنبيهات الصلاة، اسمح لأقم بالعمل دون قيود من البطارية.', style: const TextStyle(fontSize: 12, color: AppColors.textMuted)),
    const SizedBox(height: 12), OutlinedButton.icon(onPressed: onOpenAll, icon: const Icon(Icons.settings_rounded), label: const Text('إعدادات البطارية')), const SizedBox(height: 8), OutlinedButton.icon(onPressed: onOpenAutoStart, icon: const Icon(Icons.play_arrow_rounded), label: const Text('التشغيل التلقائي')), const SizedBox(height: 8), OutlinedButton.icon(onPressed: onOpenManufacturer, icon: const Icon(Icons.phone_android_rounded), label: const Text('إعدادات الشركة المصنّعة')),
  ])));
}
