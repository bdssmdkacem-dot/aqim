...
class NotificationInboxService {
  ...
  Future<List<AqimInboxItem>> getItems() async {
    ...
    for (final value in raw) {
      try {
        items.add(
            AqimInboxItem.fromJson(jsonDecode(value) as Map<String, dynamic>));
      } catch (_) {}
    }
  ...
  Future<void> markAllRead() async =>
      _replace((item) => item.copyWith(read: true));
  ...
  Future<void> remove(String id) async {
    ...
    final prefs = await SharedPreferences.getInstance();
    await prefs.setStringList(
        _key, remaining.map((item) => jsonEncode(item.toJson())).toList());
  }
  ...
  Future<int> unreadCount() async =>
      (await getItems()).where((item) => !item.read).length;
