...
class NotificationService {
  ...
  Future<void> scheduleWeeklySummary(String text) async {
    ...
    await _plugin.zonedSchedule(id: _weeklySummaryId, title: 'ملخص أسبوعك في أقيم 🌙', body: text, scheduledDate: first, notificationDetails: const NotificationDetails(android: AndroidNotificationDetails('aqim_weekly_summary', 'ملخص الأسبوع', channelDescription: 'ملخص أسبوعي لتقدمك في الصلاة', importance: Importance.defaultImportance, priority: Priority.defaultPriority, playSound: true)), androidScheduleMode: AndroidScheduleMode.exactAllowWhileIdle, matchDateTimeComponents: DateTimeComponents.dayOfWeekAndTime, payload: 'weekly_summary');
  }
  ...
  Future<void> _scheduleWakeAlarm({required int id, required String title, required String body, required DateTime scheduledDate, required String soundName, required String payload}) async {
    ...
    await _plugin.zonedSchedule(id: id, title: title, body: body, scheduledDate: tzTime, notificationDetails: NotificationDetails(android: AndroidNotificationDetails(channelId, soundName == _jumuahAlarmSound ? 'منبّه صلاة الجمعة' : 'منبّه الاستعداد — $soundName', channelDescription: soundName == _jumuahAlarmSound ? 'منبّه صوتي خاص بصلاة الجمعة' : 'منبّه صوتي قبل الصلاة لمساعدتك على الاستعداد', importance: Importance.max, priority: Priority.max, category: AndroidNotificationCategory.alarm, fullScreenIntent: true, playSound: true, sound: RawResourceAndroidNotificationSound(soundName), enableVibration: true, visibility: NotificationVisibility.public)), androidScheduleMode: AndroidScheduleMode.exactAllowWhileIdle, payload: payload);
  }
  ...
  Future<void> _scheduleFajrWakeAlarms({required DateTime finalAlarmTime, required int beforeMinutes, required DateTime now}) async {
    final stages = [(offsetMinutes: 10, soundName: 'alarm_fajr_1', title: 'اقترب وقت الفجر', typeOffset: 0), (offsetMinutes: 5, soundName: 'alarm_fajr_2', title: 'استعد لصلاة الفجر', typeOffset: 3), (offsetMinutes: 0, soundName: 'alarm_fajr_3', title: 'حان الاستعداد الأخير لصلاة الفجر', typeOffset: 4)];
    for (final stage in stages) {
      final scheduledDate = finalAlarmTime.subtract(Duration(minutes: stage.offsetMinutes));
      if (!scheduledDate.isAfter(now)) continue;
      await _scheduleWakeAlarm(id: _idFor(Prayer.fajr, stage.typeOffset), title: stage.title, body: 'تبقّى $beforeMinutes دقيقة على صلاة الفجر.', scheduledDate: scheduledDate, soundName: stage.soundName, payload: Prayer.fajr.name);
    }
  }
  ...
  Future<void> _scheduleAdhan({required int id, required String title, required String body, required DateTime scheduledDate, required String payload}) async {
    final tzTime = tz.TZDateTime.from(scheduledDate, tz.local);
    await _plugin.zonedSchedule(id: id, title: title, body: body, scheduledDate: tzTime, notificationDetails: const NotificationDetails(android: AndroidNotificationDetails('aqim_adhan', 'الأذان', channelDescription: 'تنبيه عند دخول وقت الصلاة', importance: Importance.high, priority: Priority.high, playSound: true, sound: RawResourceAndroidNotificationSound('adhan'))), androidScheduleMode: AndroidScheduleMode.exactAllowWhileIdle, payload: payload);
  }
  ...
  Future<void> _scheduleCheckIn({required int id, required String title, required String body, required DateTime scheduledDate, required String payload}) async {
    final tzTime = tz.TZDateTime.from(scheduledDate, tz.local);
    await _plugin.zonedSchedule(id: id, title: title, body: body, scheduledDate: tzTime, notificationDetails: const NotificationDetails(android: AndroidNotificationDetails('aqim_missed_prayer', 'الصلوات الفائتة', channelDescription: 'تنبيه عند عدم تسجيل الصلاة بعد انتهاء وقتها', importance: Importance.max, priority: Priority.max, category: AndroidNotificationCategory.reminder, playSound: true, enableVibration: true, visibility: NotificationVisibility.public)), androidScheduleMode: AndroidScheduleMode.exactAllowWhileIdle, payload: payload);
  }
}
