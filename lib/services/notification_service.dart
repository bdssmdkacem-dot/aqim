...
class NotificationService {
  ...
  Future<void> _scheduleAdhan({required int id, required String title, required String body, required DateTime scheduledDate, required String payload}) async {
    ...
    switch (alertMode) {
      case 'vibrate': channelSuffix = 'vibrate'; soundName = null; enableVibration = true; playSound = false; break;
      case 'ringtone': channelSuffix = 'ringtone'; soundName = null; enableVibration = true; playSound = true; break;
      case 'adhan': default: channelSuffix = 'adhan_$selectedSound'; soundName = selectedSound; enableVibration = true; playSound = true;
    }
    await _scheduleExact(id: id, title: title, body: body, scheduledDate: scheduledDate, payload: payload, details: NotificationDetails(android: AndroidNotificationDetails('aqim_adhan_${_channelVersion}_$channelSuffix', 'الأذان', channelDescription: alertMode == 'vibrate' ? 'تنبيه دخول وقت الصلاة بالاهتزاز فقط' : alertMode == 'ringtone' ? 'تنبيه دخول وقت الصلاة برنة الهاتف' : 'الأذان عند دخول وقت الصلاة — الصوت المختار من الإعدادات', importance: Importance.max, priority: Priority.max, category: AndroidNotificationCategory.alarm, fullScreenIntent: true, playSound: playSound, sound: soundName == null ? null : RawResourceAndroidNotificationSound(soundName), audioAttributesUsage: AudioAttributesUsage.alarm, channelBypassDnd: notificationPolicyAccessGranted, enableVibration: enableVibration, visibility: NotificationVisibility.public)));
  }
