...
class AppState extends ChangeNotifier {
  ...
  Future<void> loadPrayerTimes() async {
    ...
    try {
      ...
      if (lat == null || lng == null) {
        notificationIssue =
            'تعذّر تحديد موقعك — تأكد من تفعيل خدمة الموقع ومنح صلاحية الوصول له.';
        timesLoading = false;
        notifyListeners();
        return;
      }
      ...
      var times =
          await PrayerTimesService.fetchToday(latitude: lat, longitude: lng);
      usingOfflineTimes = false;
      if (times == null) {
        times = OfflinePrayerTimesService.calculateToday(
      ...
      try {
        ...
        if (!NotificationService.instance.notificationsPermissionGranted) {
          ...
          if (granted &&
              NotificationService.instance.notificationsPermissionGranted) {
            await NotificationService.instance.scheduleAllForToday(times,
                beforeMinutes: beforeMinutes,
                afterMinutes: afterMinutes,
                adhanEnabled: adhanEnabled);
            await NotificationService.instance
                .scheduleWeeklySummary(_weeklySummaryText());
            notificationIssue = null;
            debugPrint(
                'Notification permission granted and prayer notifications scheduled.');
          } else {
            notificationIssue =
          ...
          await NotificationService.instance
              .scheduleWeeklySummary(_weeklySummaryText());
          await NotificationService.instance.refreshPermissionStatus();
          notificationIssue = NotificationService
                  .instance.notificationsPermissionGranted
              ? null
              : 'إشعارات أقم محظورة من إعدادات الهاتف. اسمح للتطبيق بإرسال الإشعارات ثم اضغط «إعادة المحاولة»."';
          debugPrint(
              'Prayer notifications scheduled: ${NotificationService.instance.notificationsPermissionGranted}');
        }
      } catch (error) {
        notificationIssue =
            'تعذّر جدولة الإشعارات. تحقق من صلاحية الإشعارات وإعدادات البطارية ثم أعد المحاولة.';
  ...
  Future<void> refreshNotificationStatus() async {
    try {
      ...
      notificationIssue = enabled
          ? (realTimes == null ? 'جارٍ تحميل أوقات الصلاة.' : null)
          : 'إشعارات أقم محظورة من إعدادات الهاتف. اسمح للتطبيق بإرسال الإشعارات ثم اضغط «إعادة المحاولة»."';
    } catch (_) {
      notificationIssue =
          'تعذّر التحقق من حالة الإشعارات. اضغط «إعادة المحاولة»."';
    }
    notifyListeners();
  }
  ...
  Future<void> init() async {
    ...
    if (onboardingComplete)
      unawaited(
          ReviewService.instance.maybeRequestReview(currentWeek: currentWeek));
  }
    ...
    _clockTimer = Timer.periodic(const Duration(minutes: 1), (_) async {
      ...
      if (currentWeek != oldWeek)
        unawaited(ReviewService.instance
            .maybeRequestReview(currentWeek: currentWeek));
      notifyListeners();
    });
  }
  ...
  String get removeAdsPriceLabel =>
      PurchaseService.instance.removeAdsProduct?.price ?? '\$19';
  ...
  void _rolloverIfNewDay() {
    ...
    if (lastOpenDate != today) {
      final doneCount =
          _allPrayers.where((p) => todayStatus[p] == PrayerStatus.done).length;
      ...
      dailyPrayerHistory[lastOpenDate!] =
          Map<Prayer, PrayerStatus>.from(todayStatus);
  ...
  void _recomputeUpcoming() {
    ...
    for (final prayer in activePrayers) {
      ...
      if (t.isAfter(now)) {
        todayStatus[prayer] =
            prayer == next ? PrayerStatus.upcoming : PrayerStatus.pending;
  ...
  void _persist() {
    ...
    _prefs.setStringList(
        'today_status', todayStatus.values.map((s) => s.name).toList());
    _prefs.setStringList('today_reasons',
        todayReasons.entries.map((e) => '${e.key.name}:${e.value}').toList());
    _prefs.setStringList(
        'history', weekHistory.map((e) => e.toString()).toList());
    _prefs.setStringList('miss_tally',
  ...
  void _persistDailyHistory() {
    _prefs.setStringList('daily_history',
  ...
  void _persistDailyPrayerHistory() {
    _prefs.setStringList(
        ...
        dailyPrayerHistory.entries
            .map((entry) =>
                '${entry.key}|${entry.value.entries.map((e) => '${e.key.name}=${e.value.name}').join(',')}')
            .toList());
  }

  Future<void> _persistMissTally() async {
    await _prefs.setStringList('miss_tally',
        missTally.entries.map((e) => '${e.key.name}:${e.value}').toList());
  }
