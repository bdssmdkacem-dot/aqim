...
class _DayArcPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < prayers.length; i++) {
      ...
      points.add(Offset(margin + usable * t, baseY - math.sin(t * math.pi) * 29));
    }

    final arc = Path()..moveTo(points.first.dx, points.first.dy);
    for (var i = 0; i < points.length - 1; i++) {
      final a = points[i], b = points[i + 1];
      arc.quadraticBezierTo((a.dx + b.dx) / 2, math.min(a.dy, b.dy) - 3, b.dx, b.dy);
    }
    ...
    for (var i = 0; i < prayers.length; i++) {
      ...
      final radius = upcoming ? 29.0 : 12.0;

      if (upcoming) {
        canvas.drawCircle(center, radius + 10, Paint()..color = AppColors.gold.withValues(alpha: .18));
      ...
      _label(canvas, prayer.arabicName, center.dx, center.dy + radius + 11, labelColor, upcoming ? 17 : 12.5, bold: upcoming || missed);
      final time = timeLabelFor?.call(prayer) ?? prayer.mockTime;
      _label(canvas, time, center.dx, center.dy + radius + (upcoming ? 37 : 28), (missed ? AppColors.ember : Colors.white).withValues(alpha: .88), upcoming ? 12 : 11, bold: missed);
    }
  }
