  ...
  Widget build(BuildContext context) {
    ...
    final cells = List<DateTime?>.generate(offset, (_) => null)
      ..addAll(List.generate(
          days, (i) => DateTime(_month.year, _month.month, i + 1)));
      ...
      child: Column(children: [
            ...
            child: AnimatedSwitcher(
              ...
              transitionBuilder: (child, animation) => SlideTransition(
                position: Tween<Offset>(
                        begin: Offset(_direction > 0 ? 0.25 : -0.25, 0),
                        end: Offset.zero)
                    .animate(animation),
              ...
              child: Text('${_months[_month.month - 1]} ${_month.year}',
                  ...
                  style: Theme.of(context).textTheme.titleMedium?.copyWith(
                      color: AppColors.goldSoft, fontWeight: FontWeight.w900)),
            ),
          ),
          IconButton(
              onPressed: () => _changeMonth(1),
        ...
        GestureDetector(
          ...
          child: AnimatedSwitcher(
            ...
            transitionBuilder: (child, animation) => SlideTransition(
                position: Tween<Offset>(
                        begin: Offset(_direction > 0 ? 0.18 : -0.18, 0),
                        end: Offset.zero)
                    .animate(animation),
                child: FadeTransition(opacity: animation, child: child)),
            child: Column(
                        ...
                        child: Container(
                          decoration: BoxDecoration(
                              color: event != null
                                  ...
                                  : done
                                      ? AppColors.sage.withOpacity(.16)
                                      : AppColors.ink.withOpacity(.24),
                              borderRadius: BorderRadius.circular(10),
                              border: Border.all(
        ...
        Row(mainAxisAlignment: MainAxisAlignment.center, children: const [
          Icon(Icons.event_available_rounded, size: 11, color: AppColors.gold),
          SizedBox(width: 4),
          Text('مناسبة دينية',
              style: TextStyle(fontSize: 9, color: AppColors.inkSoft)),
          SizedBox(width: 12),
          Icon(Icons.touch_app_rounded, size: 11, color: AppColors.goldSoft),
          SizedBox(width: 4),
          Text('اضغط على اليوم',
              style: TextStyle(fontSize: 9, color: AppColors.inkSoft))
        ]),
...
class _DayDetails extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final times = lat == null || lng == null
        ? null
        : OfflinePrayerTimesService.calculateForDate(
            date: date, latitude: lat, longitude: lng);
    final historyKey =
        '${date.year}-${date.month.toString().padLeft(2, '0')}-${date.day.toString().padLeft(2, '0')}';
    ...
    return SafeArea(
        child: Padding(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              ...
              Text(
                  '${_dayName(date)} ${date.day} ${_monthName(date.month)} ${date.year}',
              ...
              if (event != null) ...[
                ...
                Container(
                    ...
                    child: Column(children: [
                      ...
                      const SizedBox(height: 6),
                      Text('${event.title} لسنة ${event.date.year}',
                          textAlign: TextAlign.center,
                      ...
                      Text(
                          '${event.hijri}${event.provisional ? ' • موعد متوقع' : ''}',
              ...
              if (times == null)
                const Padding(
                    ...
                    child: Text(
                        'فعّل الموقع في أقم لعرض أوقات الصلوات لهذا اليوم.',
                        textAlign: TextAlign.center))
              else
                ...Prayer.values.map((p) {
                  final t = times[p]!;
                  ...
                  return ListTile(
                      ...
                      leading: Icon(Icons.mosque_rounded,
                          color: status == PrayerStatus.done
                              ? AppColors.sage
                              : AppColors.goldSoft),
