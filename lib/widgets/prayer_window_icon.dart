...
/// Returns the current period of the day based on the device time.
...
/// Overlay tint used on the mosque background.
...
class _WindowPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawLine(
      Offset(sideMargin, h * 0.92),
      Offset(w - sideMargin, h * 0.92),
      gold,
    );
    ...
    if (_isNight) {
      ...
      canvas.drawCircle(
        Offset(w * 0.66, archTop + h * 0.04),
        1.4,
        starPaint,
      );

      canvas.drawCircle(
        Offset(w * 0.74, archTop + h * 0.14),
        1.0,
        starPaint,
      );

      canvas.drawCircle(
        Offset(w * 0.60, archTop + h * 0.18),
        1.0,
        starPaint,
      );
    } else {
    ...
    canvas.drawLine(
      Offset(domeCenter.dx - domeR, domeCenter.dy),
      Offset(domeCenter.dx - domeR, baseY),
      gold,
    );
